...
void Agent::SetPathFindingTarget(Node _target)
{
	pathAlgorithm->SetGoal(_target);
}


void Agent::ApplyPathFinding(Grid* maze)
{
	pathAlgorithm->ResetNodes();
	pathAlgorithm->SetStart(Node(maze->pix2cell(this->position),false));
	pathAlgorithm->findPath(this, maze);
}
...
void Agent::Update(float dtime, SDL_Event *event, Grid grid)
{
	...
	steering_behaviour->applySteeringForce(this, dtime, grid);
